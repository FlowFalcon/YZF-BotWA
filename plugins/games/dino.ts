...
const FRAMED_DINO_HTML = `<!DOCTYPE html>
<html lang="id">
<head>
  <style>
    * {
  ...
  <script>
    (function() {
      ...
      Runner.events = {
        ...
        WEBKIT_ANIM_END: 'webkitAnimationEnd',
        KEYDOWN: 'keydown',
        KEYUP: 'keyup',
        MOUSEDOWN: 'mousedown',
        MOUSEUP: 'mouseup',
      ...
      Runner.prototype = {
        ...
        adjustDimensions: function() {
          ...
          var padding = Number(boxStyles.paddingLeft.substr(0, boxStyles.paddingLeft.length - 2)) || 0;
        ...
        playIntro: function() {
          if (!this.started && !this.crashed) {
            ...
            var onEnd = this.startGame.bind(this);
            this.containerEl.addEventListener(Runner.events.ANIM_END, onEnd, {once: true});
            this.containerEl.addEventListener(Runner.events.WEBKIT_ANIM_END, onEnd, {once: true});
        ...
        onKeyUp: function(e) {
          ...
          if (this.isRunning() && isJumpKey) {
            ...
            if (Runner.keycodes.RESTART[keyCode] || (e.type == Runner.events.MOUSEUP && e.target == this.canvas) ||
...
const dinoCommand: Command = {
  name: 'dino',
  aliases: ['dinorun', 'dinorunner'],
  category: 'games',
  description: 'Memainkan Dino Runner melalui AIRich.',
  cooldownMs: 1_000,
  async run(context) {
    await context.replyAIRich(
      htmlPrimitiveMessage({
        html: FRAMED_DINO_HTML,
        caption: '🦖 Dino Runner',
        responseId: `dino-${randomUUID().slice(0, 8)}`,
      }),
